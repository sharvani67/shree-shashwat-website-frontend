...
const CheckoutPage = () => {
        ...
        const fetchProduct = async () => {
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    // Include the selected weight option in the product data
                    setProduct({
                        ...productData,
                        id,
    ...
    //         // Add order details for the single product
    //         const orderItem = {
    //             ...product,
    ...
    //         // Save address to Firestore under user's addresses
    //         const addressData = {
    //             ...shippingAddress,
    //             address_id: uuidv4(),
    //             isDefault: true,
    ...
    const handlePlaceOrder = async () => {
        ...
        try {
            ...
            // Proceed with order creation using the userId (either existing or newly created)
            await createRazorpayOrder(amount, userId);
    ...
    const createRazorpayOrder = async (amount, userId) => {
        ...
        try {
            ...
            await handleRazorpayScreen(response.data.amount, amount, userId);
        } catch (error) {
    ...
    const handleRazorpayScreen = async (amountInPaise, originalAmount, userId) => {
        ...
        // Get user details - either from currentUser or formData for new users
        const userFullName = currentUser?.fullName || formData.fullName;
        const userEmail = currentUser?.email || formData.email;
        const userPhone = currentUser?.phone || formData.phone;
        ...
        const options = {
            ...
            description: "Payment to INFAB AGRO FOODS PRIVATE LIMITED",
            image: Logo,
            handler: async function (response) {
                await createOrderInDatabase(response.razorpay_payment_id, originalAmount, userId);
            },
            prefill: {
                name: userFullName,
                email: userEmail,
    ...
    // const createOrderInDatabase = async (paymentId, amount, userId) => {
    ...
    //                 // discount: "",
    //                 // tax: "",
    //                 // hsn: orderItem.hsn || 441122 // Default HSN code if not provided
    //             }],
    ...
    //         const totalQuantity = orderItem.quantity || 1; // Default quantity fallback
    ...
    const createOrderInDatabase = async (paymentId, amount, userId) => {
        ...
        try {
            ...
            if (saveToAccount && userId) {
                ...
                if (userAddressDoc.exists()) {
                    // Get existing addresses and mark all as non-default
                    updatedAddresses = (userAddressDoc.data().addresses || []).map(addr => ({
                        ...addr,
                        isDefault: false
                    }));
            ...
            const emailSent = await sendOrderConfirmationEmail(
                ...
                orderId,
                amount,
                orderItem // Changed from cartItems to orderItem
            );
