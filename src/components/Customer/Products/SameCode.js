...
import { doc, setDoc, getDoc, updateDoc, arrayUnion, query, orderBy } from "firebase/firestore";
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        const productList = querySnapshot.docs.map((doc) => {
          ...
          let priceToUse = parseFloat(data.price) || 0;
          let originalPriceToUse = parseFloat(data.originalPrice);
          let weightToUse = data.weight || "";

          if (data.weightOptions && data.weightOptions.length > 0) {
            priceToUse = parseFloat(data.weightOptions[0].price) || 0;
            originalPriceToUse = parseFloat(data.weightOptions[0].originalPrice);
            weightToUse = data.weightOptions[0].weight || "";
          }
        ...
        console.log("Fetched and processed products (MasalaPastes):", productList);
      } catch (error) {
